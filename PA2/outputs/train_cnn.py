...
class ConvNet(object):
	...
	def model_5(self, X, hidden_size, is_train):
		...
		h1 = tf.contrib.layers.dropout(h1, keep_prob=0.5, is_training=is_train)

		w1 = tf.get_variable("W1", shape=[hidden_size, hidden_size], initializer=initializer)
		b1 = tf.get_variable("b1", shape=[hidden_size], initializer=initializer)

		h = tf.nn.relu(tf.matmul(h1,w1) + b1)

		h = tf.contrib.layers.dropout(h, keep_prob=0.5, is_training=is_train)
